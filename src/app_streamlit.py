...
from figtabminer import pdf_ingest, figure_extract, table_extract, caption_align, ai_enrich, package_export, utils, config, plot_digitize
    ...
    if st.session_state.items:
        ...
        with col1:
            ...
            selected_idx = st.radio(
                "Select Item", 
                range(len(items)), 
                format_func=lambda i: f"{items[i]['item_id']} ({items[i]['type']}) - {items[i]['ai_annotations'].get('subtype', '')}"
            )
                        ...
                        try:
                            ...
                            if m_page < ingest["num_pages"]:
                                ...
                                if page_img is not None:
                                    ...
                                    # Clamp
                                    bx0=int(max(0, m_x0)); by0=int(max(0, m_y0))
                                    bx1=int(min(w, m_x1)); by1=int(min(h, m_y1))
