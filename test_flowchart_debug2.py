#!/usr/bin/env python3
"""Debug script for flowchart detection."""
...
# Save for inspection
cv2.imwrite('flowchart_test.png', flowchart_image)
print("Saved flowchart_test.png")
...
print(f"Edge ratio in range (0.005, 0.4)? {0.005 < edge_ratio < 0.4}")
...
for contour in contours:
    ...
    if peri > 0:
        ...
        if len(approx) == 4:
            rectangles += 1
            print(f"Found rectangle with {len(approx)} vertices")

print(f"Number of rectangles detected: {rectangles}")

# Test the method
is_flowchart = merger._is_flowchart(flowchart_image)
print(f"Is flowchart? {is_flowchart}")

# Check conditions
if lines is not None:
    print(f"Condition: rectangles >= 2 and lines >= 2: {rectangles >= 2 and len(lines) >= 2}")
